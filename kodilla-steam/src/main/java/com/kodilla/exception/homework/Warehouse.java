...
/*        List<Order> order = orderList
        ...
        //return orderList.stream().filter(o -> o.getNumber().contains(number)).findFirst().orElseThrow(()-> new OrderDoesntExistException());
        //return orderList.stream().filter(o -> o.getNumber().contains(number)).findFirst().orElseThrow(OrderDoesntExistException::new);
        return orderList.stream().filter(o -> o.getNumber().contains(number)).findFirst().orElseThrow(()-> new Exception("To jest mój błąd"));
    }
}
